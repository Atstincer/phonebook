...
const App = () => {
  ...
  function udContacto() {
    ...
    contactService
      ...
      .then(response => {
        setPersons(
          persons.map(person =>
            person.id === contacto.id ? response.data : person,
          ),
        )
  ...
  const onSubmit = event => {
    ...
    if (alreadyExist()) {
      ...
      const ok = confirm(
        'El contacto ya existe....seguro que desea actualizar su numero?',
      )
  ...
  return (
    <div>
      ...
      <PersonForm
        onSubmit={onSubmit}
        name={newName}
        onNameChange={onNewNameAdded}
        phone={newPhoneNumber}
        onPhoneChange={onNewPhoneNumberAdded}
      />
      <h3>Numbers</h3>
...
const MensajeError = ({ mensaje }) => {
  ...
  return <div className="mensajeError">{mensaje}</div>
}
